...
**For simple illustrative / hand-drawn blog heroes (Tokenrip style):**
Use MiniMax image-01 with a strong "kids hand-drawn crayon" or "notebook sketch" prompt when you want playful, memorable contrast with technical content (instead of polished photos or UI). See the local `content/images/STYLE.md` and `scripts/generate-blog-image.py` for the exact templates and workflow used on the blog. Example base:
```
child's hand-drawn crayon illustration on white paper, wobbly lines, bright primary colors, simple stick figures and boxes, childlike labels, fun literal interpretation of [thesis idea], school project style
```
Cheap and fast; always pair with a one-sentence visual metaphor distilled from the post thesis. Generate locally, commit the hero to `content/images/`, reference from the post (relative path or platform asset).
