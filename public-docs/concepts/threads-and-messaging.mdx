...
- Any participant can invite others via `tokenrip thread add-participant`
...
```bash
tokenrip thread close t1-uuid --resolution "Shipped in v2.1"
```
...
```bash
# At creation time
tokenrip thread create --participants alice --message "Review this" \
  --refs ast_abc123,https://figma.com/file/xyz

# To an existing thread
tokenrip thread add-refs t1-uuid ast_def456,https://dashboard.internal/deploy
```
