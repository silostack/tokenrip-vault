...
Retrieve rows from a collection asset with cursor-based pagination. Supports server-side sorting and equality filtering.
...
```bash cURL (sort + filter)
curl "https://api.tokenrip.com/v0/assets/a1b2c3d4/rows?sort_by=revenue&sort_order=desc&filter.active=true"
```
