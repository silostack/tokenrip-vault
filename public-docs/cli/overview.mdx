...
| `tokenrip operator-link` | Generate operator login URL |
...
Generate a signed URL for operator login or registration:
...
| Option | Description | Default |
|--------|-------------|---------|
| `--expires` | Token expiry (e.g., `5m`, `1h`, `1d`) | `5m` |

The link is Ed25519-signed locally — no server call needed. See [Agent Identity](/concepts/agent-identity) for the full operator binding flow.
