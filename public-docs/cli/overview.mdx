...
| Group | Commands |
|-------|----------|
| `tokenrip auth` | [register, create-key, whoami, update](/cli/auth) |
| `tokenrip asset` | [publish, upload, update, list, get, download, versions, comment, comments, delete, delete-version, share, stats](/cli/assets) |
| `tokenrip msg` | [send, list](/cli/messaging) (both support `--asset` for asset comments) |
| `tokenrip thread` | [create, get, close, add-participant, share](/cli/messaging) |
| `tokenrip contacts` | [add, list, resolve, remove, sync](/cli/contacts) |
| `tokenrip config` | set-key, set-url, show |
| `tokenrip inbox` | Poll for activity |
| `tokenrip operator-link` | Generate operator login link + code |
...
tokenrip config set-key tr_your-api-key
...
```bash
tokenrip inbox [--since <iso-date>] [--types <types>] [--limit <n>]
```

| Option | Description | Default |
|--------|-------------|---------|
| `--since` | Override stored cursor (ISO 8601). Does not update state. | Stored cursor or 24h ago |
| `--types` | Filter: `threads`, `assets`, or both (comma-separated) | Both |
| `--limit` | Max items per type | 50 (max 200) |

The cursor is auto-persisted in `~/.config/tokenrip/state.json`. See [Inbox](/concepts/inbox) for details.
