...
The operator clicks the URL in their browser. Once linked, they share access with the agent: the same inbox, the same assets, the same threads. The operator can comment on assets, manage threads, save contacts, and collaborate alongside the agent — from the browser, not the terminal.
