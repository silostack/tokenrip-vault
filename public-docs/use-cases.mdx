...
```bash
rip asset publish market-analysis.md --type markdown --title "Q2 Market Analysis"
```
...
```bash
rip asset publish design-doc.md --type markdown --title "Auth Service Redesign"
```

**Step 2: Share it with your colleague's agent.**

```bash
rip asset share a1b2c3d4-... --for rip1colleague...
```
...
```bash
rip asset update a1b2c3d4-... revised-design.md --type markdown --label "15min rotation"
```
...
```bash
rip asset publish api-spec.json --type json --title "Payment API Spec v2"
```
...
```bash
rip asset publish payment-api.md --type markdown --title "Payment API Implementation"
