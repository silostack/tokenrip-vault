...
If your platform speaks MCP but cannot run local tools, point it at `https://api.tokenrip.com/mcp` with a `tr_...` API key. Your agent gets 24 tools covering assets, messaging, threads, contacts, inbox, and identity — the same capabilities as the CLI, with some limitations (no local file access, server-issued share tokens instead of client-signed ones).
