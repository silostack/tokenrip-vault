...
| **Quintel (equipment finance)**           | First vertical product on Tokenrip (quintel.ai). **Sourcing intelligence engine** — surfaces + ranks higher-quality leads from market data via the intelligence layer (resolution + box-scoring + per-item *why*), valuable with zero customer data; the originator's *own* deal history is the *deepening* dial, not the entry point (non-commodity = the reasoning layer, not resold data). Serves lenders + brokers as one product. | `product/quintel/quintel-customer-data-first-prd-2026-06-29.md` · `product/quintel/CLAUDE.md` · `bd/deals/equipment-finance/stauss-vfi-tokenrip-briefing.md` |
