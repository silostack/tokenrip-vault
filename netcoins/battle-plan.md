...
- **Product direction (23 Sept):** Fraser's [consumer-app artifact v0.4.1](https://claude.ai/artifact/GUFQqVGxbCdkQTaWUQG6t7) is indexed in [[netcoins/product-direction]]. It describes a broad 12-week app while the 22 Sept Netcoins operator call scoped a narrow USDC yield integration. Confirm whether these are separate workstreams and preserve objective, narrower C$200K milestone acceptance in the DA.
...
| 5   | Next 2 weeks | Draft the integration and transition plan ourselves and hand it to Fraser: milestone = compile, deploy to their environment, one on-chain transaction, runbooks (his words on the 08-27 call). Whoever writes the first draft defines "objective." Same doc carries the earn-out (d) roadmap items — keep them few and shippable. **09-22:** the natural milestone vehicle is the Netcoins yield integration (USDC, Morpho-style curated vault, non-custodial wallet). Acceptance must mean deployed in Surge's environment, not client go-live, because go-live depends on the regulator. | Simon           |
| 6   | Next 2 weeks | Propose the Net Revenue attribution formula for the Netcoins yield path (e.g. basis points on balances routed through RebelFi). If it isn't defined in the DA, earn-outs (a)–(c) can never be measured. **09-22:** the mechanism is now concrete (spread taken on withdrawal, sent to a RebelFi wallet). Stablecoin AUC is only ~US$1.5M, so also settle whether treasury-float yield counts. See [[netcoins/calls/netcoins-call-notes-2026-09-22]].                                                                                                                           | Simon + Alek    |
...
- **Candidates (as of 09-18):** [[netcoins/counsel/matt-loeffelholz]] shows strong substance, has no quote yet and bills a blended hourly rate. [[netcoins/counsel/erin-camp]] offers US$200/hr, staff at $40–75, a cap tied to target close and billing on the closing funds flow, but has not yet shown substance. Rain conflict and Upwork ToS still need checking. **Gate:** Erin's top-3 LOI issues vs. Matt's. Choose before the DA lands.
- **Expense ask, now concrete:** LOI §13 lets the DA override each-pays-own. Put counsel + CPA fees on the closing funds flow (Surge pays at close or by note). The Surge raise is meant to close before ours, so the cash exists. Carry this into our first DA markup whichever lawyer we hire.
