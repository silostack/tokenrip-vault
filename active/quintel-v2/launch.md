...
- **Reply machine and the rest of the engine (09-24):** designed in `outbound.md`: sorting automated from day one (classify, verify, suppress, acknowledge, ping, ledger row), the conversation stays David's by hand until 100 labeled positives; David's acknowledgment spec as timeline rules with an availability config; the sender-identity question (T32); build order against 10-06 in its §7.
