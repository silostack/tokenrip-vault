...
6. **CRM collision returns.** When David marks a row `existing customer` or `contacted within nine months` or `other AE`, the company goes to suppression with reason `pcf_book`, and the send machine stops touching it. The lead itself is not dead: it parks with the reason code and, under the rule in §5f, may route to a second originator. Two flags, suppressed and routable, on two different objects. This closes the loop `call-analysis-2026-09-22.md` §3 opened: collisions are caught at handoff and cost nothing, but only if the handoff writes back.
...
**Superseded 09-26 (`build.md` §10):** the handoff layer (the register) is built in Ironmark, a standalone system; Quintel is Ironmark's first lead source behind an adapter and receives outcomes as labels. The two-layer argument below still holds; the layers now live in two systems. The paragraph on reusing Quintel's tables becomes a design reference for Ironmark's register, not a build instruction.
...
> Thanks for getting back to me, [first name]. Someone from one of the lenders we work with will call you [today / tomorrow morning / Monday] at [phone]; I'll make sure they have your note. If a specific time works better, you can pick one here: [link]. Nothing is needed from you before the call.
>
> [Ironmark sender]
> [postal address] · [opt-out line]

Variant Q adds one sentence answering the question from the focus-group list (the "how did you get my information" and "what is the catch" answers, once Saturday says which line owners believe). Variant I-named puts "David LaSaee, with one of the lenders we work with, will call you" in the first sentence; it is only usable if the collision rate is low enough that a correction email is rare (§5f). Variant I-ask replaces the first sentence with "Mind if I have someone from one of the lenders we work with call you [today]? Reply 'sure' and they will," and waits for the reply before pinging. Variant D (David as sender) drops the introduction and speaks in the first person. All are fixed copy with five slots; nothing is generated.
...
| **C. Two-step (recommended).** Instant acknowledgment from the Ironmark mailbox promising a call from "one of the lenders we work with," nobody named or copied; ledger pings David; after the CRM check the thread is forwarded to his Providence address, or the lead is routed elsewhere (§5f); the caller introduces themselves; application through the lender's own channel | instant and deterministic; the CRM check happens before Providence enters the thread; the owner was told a lender's rep would call, so Providence on the phone is expected, not a switch | needs the site to connect the two names for the legitimacy check (a funding-partners line that lists Providence), a real Ironmark person as sender, and David to say the same sentence on every call |

Under C the chain is: reply → acknowledgment (Ironmark sender, a lender's call promised, no name, no Cc) → ledger ping → CRM check → thread forwarded to David at Providence, or routed to a second originator on a collision (§5f) → the caller identifies their lender and says the partner line → the lender's credit application from the lender's own email or link. Ironmark never holds an SSN, a bank statement or a guarantor's ID. An earlier version of this section had David copied on an Ironmark inbox; that contradicts "he is with the lender" and is dropped.
...
### 5f. Collisions become routable leads, which settles the sender question

Simon's point (09-25): with an Ironmark sender, a positive reply that turns out to be off-limits at Providence (existing customer, contacted in the last nine months, another AE's application) is not dead. The lead belongs to Ironmark's funnel, and the routing table (`funnel.md` §7) can send it to a second originator. Under the David-as-sender chain the same reply is waste: David cannot call a colleague's customer, and nobody else can either, because the owner replied to "David." Colorado's dial lists showed about a fifth of rows already in Providence's book (`active/flpool/TODO.md`, 6 of 28 on the first partial). That fifth is supply for the second originator (T31, GME / National Truck Loans in Florida) rather than a cost of the collision check. This is the structural reason for a founder's name on the mailboxes, on top of the trust and header reasons in §5b.

It changes two things in the chain.
...
|---|---|---|
| funded Providence customer, or another AE's live application | to the AE of record through David, as he does today; row marked `pcf_book`, not re-routed | unchanged unless ownership says otherwise |
| contacted by Providence in the last nine months, never funded | park; re-route to a second originator **only with David's explicit yes** on the class, given once, not per lead | routable by default once the rule is in writing with the referral agreement |
| Providence passes or declines (out of box, credit, David's bandwidth) | re-route after David marks it, no question | same; this is the exclusivity window in `funnel.md` §7 |
| clear | David | David, or the routing table when there is more than one originator in the state |

Two flags, not one. Suppression stops *emailing* a collision company (no more sequences to an existing Providence customer). Routability is a separate state on the lead object. The ledger carries both, and a suppressed company can still hold a routable lead.
...
| A collision lead re-routed to Providence's competitor on the strength of David's own CRM check | the "stealing our accounts" story with another lender's name on it; lands on David | typed rule in §5f: funded customers and live applications never re-route in phase 1; the nine-month class only with David's explicit yes; passes and declines freely |
| Acknowledgment names David, then someone else calls | a fifth of positives get a correction email | acknowledgment promises a call from "one of the lenders we work with" and the caller introduces themselves; named variant only if the group says the name matters and collisions are rare |
...
3. The scheduling link: in the acknowledgment, after the phone promise; his Calendly or one of ours; and whether he wants the booked time on the ledger row.
   Also: that he is neither named nor copied on the acknowledgment (the thread reaches his Providence address after the CRM check; §5f says why no name), whose real name goes on the Ironmark mailboxes if the two-people chain wins, and the one sentence he says on every call to connect Ironmark to Providence (§5b, §5e).
...
7. Re-routing collisions (§5f): his yes or no, by class. Funded customers and live applications stay inside Providence; may a "contacted in the last nine months, never funded" lead go to another lender on our panel? Ask once, as a rule, after the first collisions are on the ledger, not as a hypothetical.
