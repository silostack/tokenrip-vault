...
- **The focus group tests exactly this on Saturday** (`focus-group-questions-2026-09-26.md` Q1 and Q5: sender A/B/C, and "would you rather the first email came from the lender or from a company that sends you to the lender"). That is the cheapest disconfirming test in the plan and it arrives in three days. Build the acknowledgment with two template variants (sender = David, sender = Ironmark person) and let the focus group plus David pick. Do not hard-wire either.
...
Rule: **no link in the cold sequence; a link in the acknowledgment, second to the phone promise.** The line is "I'll call you today at [number from the reply or the row]; if a specific time is easier, pick one here." The link points at David's calendar, on a page that writes the booked time to the ledger row so the queue shows it. Focus group Q7 tests whether owners prefer "best number and time" or a calendar; measure booking rate over the first 50 positives and drop the link if it is under one in ten. Whose calendar is a question for David (his own Calendly, which he already uses, or one on the Ironmark domain); the row link is the requirement either way.
