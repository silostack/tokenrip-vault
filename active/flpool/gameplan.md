...
Living document. Start here. Last revised 2026-09-19, after David returned the completed FL 100 (§4.12: event tags hold at n=28, 18% dead vs 57%; tree lane not special; 12% wrong-number rows are our defect). Prior 2026-09-15, partial FL-100 + CO-100 verdicts (§4.11). Prior 2026-09-14, after Wisconsin verdicts (§4.10). Prior revisions 2026-09-11 (his follow-up reply, template, WI batch shipped) and 2026-09-10 evening, after the three-way call (transcript: `../../bd/calls/transcripts/david-lasaee-2026-09-10.md`).
...
### 4.12 FL 100 completed (09-19): the event finding holds at n=28; tree is not the problem; wrong numbers are

David returned the full sheet 09-19 (`data/raw/david_fl_completed_2026-09-19.xlsx`, scored by `scripts/c5j_verdicts.py flc` → `out/verdicts/david_fl_completed_2026-09-19.csv`; 95 unique rows after the 4 duplicate pairs and one unmatched). His note: "more yellow and red… tree services companies are showing less interest. It can be seasonal and/or perhaps the small sample is skewing the results." This workbook adds theme8 = funded customer.

**Headline (95 rows):** 43 dead · 41 voicemail/no decision · 8 existing/CRM by outcome (11 rows with LP or GM = yes) · 2 open (Accurate Transport "callback in a couple of months"; Highmark Lighting "year-end buy, talk in November, schedule set") · 1 follow-up (B&B Underground, "call Monday") · 3 scheduled callbacks marked grey (Britt's Dump Truck, 333 Tow, ASAP Onsite Septic). 26 reached a human, 14 the owner. **0 applications from the list** (All City Septic "APP in" is an existing lead, not ours). 41 rows were offered "promo rates / Q4 promos": David is now selling a Q4 promo on the phone.

- **The event tags hold, now at n=28 rather than 2.** HOT rows (EXPANSION / HIRE_OPS / NEW_DOT / CONTRACT_AWARD): 28 rows, **5 dead (18%)**, 3 open, 6 CRM. WARM lien-timing rows: 67 rows, **38 dead (57%)**, 0 open, 2 CRM. Every open door and 6 of the 8 CRM hits sit on HOT rows. Read it two ways at once: the event detector finds real prospects, and a good share of them are already Providence's. That is the strongest confirmation yet that events are the ranking signal and lien timing is not (24–48mo and 48+ bands: 48% dead each; 6–24mo: 27% dead but n=11 and both opens).
- **Tree is not special.** T_TREE: 30 rows, 14 dead (47%), 1 open, 2 CRM. Tow 52% dead, roll-off 50%, vac 38%, pump 33%. Overall 45%. Tree's dead rate is the batch's dead rate with the largest n; the yellow David saw is the lane's size, not its temperament. What tree does show: two hang-ups at the word "financing," two "we do not finance anything" households, two Spanish-language mailboxes, one number that was another company's. Seasonality is plausible (Sept–Nov is storm-cleanup season in FL, crews are busy) but the data does not separate it from base rate. Tell him: not enough to drop tree; enough to stop over-weighting it (30 of 95 was too many for one lane).
- **Twelve rows (13%) were wrong-number or wrong-company: our defect, not the market's.** Five NIS despite Twilio "valid" (Bradford Garbage, Central Florida Sitework, Ken's Tree, Crocs, Boyett), two wrong company on the line (Par Towing = Phantom Asset Recovery; Gautier = another company's name), two Spanish-only mailboxes, one blocked line, one mailbox not set up, one wrong email. Twilio line validity is not reachability. Add a second-source phone check (Places/website number vs the record number) and a language flag before the next list; David corrected four numbers by hand from the web, which is our job.
- **Turnover surfaced on its own.** Four rows are sellers, not buyers: Roll-Off Services of Florida "trying to sell the company," Jag Property Solutions "selling equipment, tough year," Fast Towing "selling his tow truck" (David asked for the truck's details to match a buyer), Temsa (a national bus vendor, mis-pooled). Private-party sellers are the turnover signal in `../quintel-v2/signals.md` §3d, seen from the calling side; the buyer of that truck is the lead.
- **Reach did not change between halves.** First 48 rows: 13 reached, 22 dead; second 47: 13 reached, 21 dead. Mobile 14/54 reached (8 owners); fixedVoip office lines 8/17 reached (4 owners, the rest gatekeepers who end the call). Five rows note AI or iPhone screening. Same ceiling as §4.11.
- **CRM overlap 11/95 (12%)**, between WI's 10% and CO's 21%. Under the clean-handoff model (`../quintel-v2/providence.md` §4) this stops being a list-hygiene problem; on the calling lab it still costs David the dials.
- **For the call:** the result is a normal hard-cold outcome plus a firm ranking rule. Next FL list = majority-HOT (events), lane-balanced (no lane above 25%), second-source phone check, no duplicates. The loss-signal arm ships on the same template.
...
| **FL 100 (intent-ranked, blind)** | **completed verdicts in 09-19** (`out/verdicts/david_fl_completed_2026-09-19.csv`, `scripts/c5j_verdicts.py flc`; partial 09-15 superseded) | 100 (95 unique) | **first batch with a real intent layer; the finding held at n.** All specialty lanes, fleets 2–7, EST. **Result (§4.12): HOT event rows 18% dead / 3 open / 6 CRM vs WARM lien-timing rows 57% dead / 0 open. Lien-age bands flat. Tree lane 47% dead = batch base rate (45%), not an outlier. 13% wrong-number rows are our defect.** 43 dead / 41 voicemail / 2 open + 1 follow-up + 3 scheduled callbacks / 11 CRM / 0 apps; 4 dup pairs in Alek's export |
