...
prepared_by: "[firm name]"
date: 2026-06-06
status: DRAFT for internal review (Simon) before sending to client
---
...
| **Q4 — Auth / attestation / governance?** | Access controlled by the hospital; **attestation happens in the provider portal** (or via CAQH), with no documented way to import an outside attestation **(H)** | Access issued through the vendor; sensitive provider data → expect the hospital's security review and data-handling agreements; attestation for API submissions undocumented **(M)** | Most controlled of the three (access request required); attestation appears to live in the portal **(M)** |
...
The consequence is simple: **there is no single, one-size integration.** Each hospital is the baseline plus its own completeness-to-spec plus its own delivery route — so the right shape is a repeatable per-hospital deployment, with the baseline engine as the durable asset that every deployment reuses. This builds on, and updates, the phased engagement already discussed; what discovery adds is a much clearer picture of what production-readiness requires for integration, and the recognition that deployments are inherently two-layer. The discovery study credits toward the build, and the team that conducted it is positioned to start on the demo now and carry through to the first hospital deployment.
