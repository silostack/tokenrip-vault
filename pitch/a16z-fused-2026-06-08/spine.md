...
- **Credentialing (AICAP):** **paid discovery done; MVP next.** Customer is **a credentialing company**, never "a hospital." The ~$22K/day is the *end-customer's* pain in the space.
- Claim **position + insight + velocity, never a finished moat** — the acting-agent + why-loop hasn't fired in either vertical.
...
3. **"git is free — the value is GitHub/CI/the network."** Tokenrip is positioned as the *GitHub* (where value accrues), and the proprietary asset is the why-graph, not the free protocol. The tagline says "git" for memorability; slides 3 and 5 say platform.
4. **"Isn't this FDE consulting in a trenchcoat?"** Config-not-code (slide 5): the spine is reused, margins climb, verticals are products, not bespoke rebuilds.
