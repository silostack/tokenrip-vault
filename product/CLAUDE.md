...
Equipment-finance deal-intelligence platform (quintel.ai), **powered by Tokenrip**. **Current direction (2026-07-14): a sourcing intelligence engine** — a personalized relevance-stream that surfaces + ranks higher-quality leads for an equipment-finance originator from market data run through the intelligence layer (entity resolution + box-scoring + a per-item *why it's relevant*), valuable with zero customer deal history; the originator's *own* deal history is the **deepening** dial (revealed-preference ranking) layered on top, not the entry point. Non-commodity by construction — the differentiation is the *reasoning layer*, not resold public data — serves both lenders and brokers as one product, pointed at a different list. Architecture = **share the evidence, privatize the score** (owner-tagged evidence; per-customer scoring; structural leak-safety). The per-customer deal-graph it accumulates *is* Tokenrip substrate — Quintel is how the substrate gets its first dense vertical. *(The 2026-07-14 sourcing-first reframe supersedes the earlier "customer-data-first" emphasis: market data + the reasoning layer lead, the book deepens. Earlier broker-first / "sell the hands" / three-archetype / naive "resell raw public signals" motions remain archived — see the [PRD](quintel/quintel-customer-data-first-prd-2026-06-29.md).)*
...
| **Quintel — canonical PRD (sourcing-first)** | [[quintel-customer-data-first-prd-2026-06-29]] |
