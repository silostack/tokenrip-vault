...
Owner-tagged evidence (`public` or `principal:P`): a shared world-graph of entities + public evidence, plus each customer's **private overlay** (their deal history / the Quintel Book). The **score is always a per-customer read** over `owner ∈ {public, P}` — so leak-safety is structural (customer A's private data is never in customer B's input set), and adding market data is just appending `public` evidence to the same ledger.

**The moat is two-part (see PRD §4A):**
- **Type 1 — per-customer lock-in:** the learned scoring function + standing pipeline + public-research entity-resolution/inference IP. Switching cost; real but defensive.
- **Type 2 — pooled network effect (the offensive moat the wedge earns into):** *anonymized, aggregated* outcomes across customers → system-wide intelligence no competitor without the pool can match (e.g. Alek's signal→actual-term correlation that tightens renewal-timing for everyone). **PayNet** (hundreds of EF lenders contribute loss data, get risk scores back; now Equifax) proves EF lenders *will* contribute private data for pooled intelligence; Filmore states the same on its FAQ. Scale-gated: **architect now (capture closed-deal metadata via the CRM pipe, contract for anonymized contribution), build after N customers, pitch only when the pool is real.**

**Raw `owner=P` private data never crosses** (that's what makes customers willing to give it); **anonymized aggregates are the pool.** The CRM integration ([[quintel-sourcing-intelligence-prd-2026-06-29#15]]) is both the BDO-proof and the data-acquisition pipe for the Type-2 pool.
...
| Doc | What |
|---|---|
| **[[quintel-sourcing-intelligence-prd-2026-06-29]]** | **Canonical PRD** — the sourcing intelligence engine (2026-07-14 reframe: market data + reasoning layer lead, the book deepens): relevance-stream, share-evidence/privatize-score, the Quintel Book + ingestion, the explainable ranker, market-intel L1–L4, v1 build scope, demo path, load-bearing assumptions |
...
| [[quintel-data-rigor-roadmap-2026-07-21]] | **Data rigor roadmap** — verified measurement-gap audit (raw-payload loss, unvalidated resolution, n=19 calibration) + sequenced fixes: Phase 0 sprint-compatible low-hanging fruit (UCC backtest, gold set, label harvest), Phase 1 continuous measurement, Phase 2 forward layer |
