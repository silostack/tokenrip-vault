---
title: "Quintel v1 — Sourcing Intelligence Engine (PRD)"
status: draft
owner: Simon
...
# Quintel v1 — Sourcing Intelligence Engine (PRD)
...
> **Framing note (2026-07-14 reframe).** Quintel leads with **sourcing** — surfacing and ranking higher-quality leads from **market data run through the intelligence layer**. That is the customer's voiced pain ("I need better leads / who do I call first"), and it is valuable with *zero* customer deal history. The customer's own deal history is the **deepening layer** (a certain-value dial that makes the intelligence much smarter), not the entry point. *(This supersedes the earlier "customer-data-first" emphasis; the filename retains that slug for link stability only.)* The escape from the commodity trap is the **reasoning layer** — entity resolution + box-scoring + per-item "why this matters to you" — **not** the raw feed and **not**, by itself, the customer's data. The market-data/signal thinking is developed in the companion **[signal portfolio](quintel-data-strategy-signal-portfolio-2026-07-13.md)** (the five originator jobs, the leading-vs-lagging temporal axis, and the sequencing implications), which this PRD now treats as primary input.
...
Quintel is a **sourcing intelligence engine for equipment-finance originators** — the people at lenders and brokers whose job is to find financeable deals. Their daily work is **prospect triage**: deciding which companies to call out of a large list, then calling down that list. Most of that effort is wasted on companies that will never fund, and they find out about real deals only after those deals are already contested. **The pain they actually voice is lead quality** — *"I need better leads, and I need to know who to call first."* Nobody asks for "more intelligence on my own data"; everybody asks for a better list.

The core product answers that directly: a **personalized, interactive market-intelligence feed** that surfaces and ranks the leads that matter to this originator — companies from their prospect universe, net-new companies that fit what they fund, market events, and the occasional real deal signal — each item shown *with the reason it's relevant to them* (it's in a sector they win, it's in a sponsor they track, it's in their sweet-spot ticket band, it just moved in the public record). Think a Bloomberg terminal for equipment finance, tuned to one desk. It is **not a raw signal feed** (real cold signals are rare; this is dense) and **not a reshuffled list** (the value the customer *sees* is the per-item reasoning).

**What keeps this out of the commodity trap is the intelligence layer, not the raw data.** Apollo and ZoomInfo sell the same market data to everyone; Quintel's differentiation is the *reasoning* on top — entity resolution across sources, box-fit scoring, and a per-item "why this matters to you." That intelligence is valuable **from day one on market data alone**, with only the coarse context an originator can describe in a conversation (their box, the sponsors and companies they watch, their sectors). Public market data is woven in **from v1** — it's what makes the feed feel alive — but always as *tailored intelligence*, never a raw feed.

**Connecting the customer's own deal history is the deepening layer — it makes the intelligence much smarter, it is not what makes it work.** Their funded book reveals what *actually* funds (vs. their stated box), which turns generic box-fit ranking into **revealed-preference ranking** and adds deal-resemblance reasons, channel intelligence, and a boundary profile. This is a **dial**: certain, positive lift that deepens relevance and hardens the *"built for you, not a newsletter"* claim — layered on a product that is already valuable without it.

The first demo's job is to make one thing undeniable: **the system understands this originator's world and hands them a better list.** Not "the ranking is provably correct" (unprovable in a demo) and not "this deal will close" (unprovable ever), but — on a single item — *"here's why this is a lead worth your time,"* tied to live market data (and, where available, their real deals), which the customer can validate on sight. **Comprehension, made visible, is the wow.**
...
## 3. The core insight: intelligence over market data, deepened by the customer's book

The pain is lead quality. The mistake is thinking *raw data* solves it — and the escape is the *intelligence layer*, of which the customer's own book is the deepest tier, not the entry point.

**The trap — selling public data ("we surface deals from the market").** Originators voice one need: better leads, and knowing who to call first. The naïve answer is to sell them more public data. But raw public data is a commodity: Apollo, ZoomInfo, and the trade newsletters all sell it; it's the same data for everyone. Leading on the *feed* invites every losing objection — *"are we all chasing the same intel?"*, *"are you giving my deal to a competitor?"*, *"what's your batting average?"* — and, for cold net-new deals specifically, rests on the least-certain question in the whole product: *does the public signal even exist?* (A multi-year backtest over EF public records surfaced only a handful of clean cold leads — real, but sparse. You cannot infer a deal from data that isn't there.)

**The escape — the intelligence layer, not the data.** What is non-commodity *by construction* is the **reasoning** Quintel runs over the market data: entity resolution across sources, box-fit scoring, and a per-item explanation of *why a lead matters to this originator*. The same public permit is a commodity as a row in a feed and a differentiated lead as *"a company in your sweet-spot sector that just filed to expand."* This is why Quintel leads with the feed's *tailoring and reasoning*, never the raw feed — and why the winning asset is the fusion + scoring + reasoning layer, not any single source (developed in the [signal portfolio](quintel-data-strategy-signal-portfolio-2026-07-13.md) §4).

**Two levers of a better list — both ranking/reasoning, not more rows.** Higher lead quality means (a) *surfacing the right companies earlier* — including **leading signals** that fire *before* the deal (permits, contract awards, new operating authority) so the originator reaches a deal before it's contested — and (b) *ranking the universe so the good ones rise and the dead patterns sink*. Neither needs the customer's private data to be valuable; both are the intelligence layer applied to market data. *(The signal portfolio's five originator jobs make this concrete: demand-origination and timing are leading-signal jobs, qualification and competitive-mapping are standing jobs — all served off market data before any book is connected.)*

**The customer's own deal history is the deepening layer — a dial, not a switch:**
- **Dial vs. switch.** Intelligence over the customer's own book is a **dial** — risk = *magnitude* of lift, floor is positive, definitely buildable. Cold-surfacing net-new deals from sparse public convergence alone is a **switch** — risk = whether the signal exists at all, outside our control, sparse. *Lead the product on the intelligence layer over market data (certain value); deal history deepens it (the dial); cold net-new convergence stays upside (the switch).*
- **Deal history = the credit team's judgment, moved to the front of the funnel.** We are not predicting cold fundability (which needs post-submission financials). We take what the customer's credit team has *already decided*, encoded in deal history, and use it to *reweight* the originator's list — closing the internal information gap. Mike's 3,500 are not cold; they are his list. He just doesn't know which to call, because he's not credit. This deepens the aim; it isn't what lets us aim.
...
- **Deal *history / metadata*** — what sector, what ticket, what structure, who referred it, which lender funded it, won/lost and why. All **cold-observable** on any prospect, and this is what deepens the ranker.

The history doesn't give us new facts about a cold prospect; it tells us **how to weight the facts we already see.** The buy box is what the customer *says* (manufacturing, $3–50M). The deal history is what they *actually do* (last 50 deals are 90% food-processing and metal-fab, zero mining despite it being "in-box," 60% via two OEMs). Same public attributes — the box weights them flat (in/out), the history gives the *real* weights.

**Why the book still matters enormously (just not first).** It is non-commodity by construction (the output differs per customer), its lift is *certain* (the data is all there), and it does five business jobs at once (§4, §9) — it's the difference between a good tool and *their* tool. The reframe is only about *order*: lead with the intelligence-over-market-data that answers the voiced pain (better leads), then deepen with the book.
...
2. **Market intelligence + the reasoning layer → a better list.** **This is the v1 baseline and the answer to the voiced pain.** The intelligence layer surfaces and ranks leads from market data — **leading signals** that fire before the deal (permits, awards, new operating authority → reach it early), qualification/box-fit that de-tire-kicks the list, net-new companies that fit what they fund, and a per-item *why* — tailored to the originator's coarse context (their box, the sponsors and companies they watch, their sectors). **Valuable with zero customer deal history.** Delivered as *tailored intelligence* (annotated, ranked, interactive), never a raw commodity feed. Its breadth grows in levels (§11).
3. **Deal history → deepens the ranking (the dial).** Their funded book turns generic box-fit into **revealed-preference ranking** and adds deal-resemblance reasons, the channel map (which OEMs/partners actually close), the down-ranked dead-pattern tail, and the **box-reality gap** ("you say mining; you've closed zero"). This hardens the *"built for you, not a newsletter"* claim — layered on a rung-2 product that already works.

**The ladder degrades gracefully on deal-data depth — which de-risks the whole sell.** Rung 2 stands on its own: with only coarse context (their box, watchlist, sectors — gathered in a conversation), the feed is already tailored and valuable, because the differentiation is the *reasoning layer*, not the customer's data. Deal history (rung 3) *deepens* the relevance (revealed-preference ranking, deal-resemblance reasons, channel intelligence); it is the upgrade path, not the entry toll. This matters because getting deal data can be slow or blocked (a new lender like Empire has little; some credit teams won't share it) — and the product must land valuable *before* that arrives. The floor is a coarse-tailored better-lead feed; deal data cranks it up.

**Why the customer pays, and what the setup fee buys.** Rung 2 wins the *"this gives me better leads"* sale. Ingesting deal history (rung 3) is what earns the setup fee and deepens the lock-in — one event that does five jobs at once: it (1) builds the custom pipeline that **justifies the $10k setup fee**, (2) hardens the **"built for you / trained on your book"** claim beyond the coarse-context tailoring (the reasoning layer already beats a newsletter; the book makes it unmistakably *theirs*), (3) is the moment the intelligence becomes unmistakably theirs, (4) starts our **data moat**, and (5) is a *standing* pipeline that keeps ingesting future deals, which is why the monthly subscription recurs and the lock-in compounds (six months in, it knows their book better than any new tool could, and leaving means it forgets everything).

## 4A. The framing — Quintel as a mounted BDO, and the two-moat model

*(Added 2026-07-14. This section reframes how the §1–§4 value is **sold** and where the durable **moat** comes from. It does not change the architecture; it changes the budget line the product competes for and names the network-effect moat the wedge earns its way into.)*

### The frame: a mounted BDO, not a lead list

The §1–§4 value can be sold as a *tool* ("a better lead list") or as *labor* ("an entry-level business-development officer"). The labor frame is materially stronger, for four reasons:

- **It changes the budget line.** A lead-list tool competes for scrutinized software budget (low hundreds to low thousands per seat). A BDO competes against a fully-loaded junior-originator hire (~$60–100k+, months to ramp, high wash-out). Against a *headcount* line, a $10k setup + monthly subscription reads as *cheap*, not expensive — and EF originators are actively asking the tool-vs-hire question ("as we grow, is this the right thing to scale with?" — Katharine, Empire).
- **It flips the evaluation question from unwinnable to winnable.** A tool is judged on *"what's your hit rate?"* — unprovable in a demo, and the objection that has dogged every sourcing pitch (Mike asked it three times). Nobody asks a junior BDO for their batting average before hiring them; they ask *"can it do the grunt work and free up my seniors?"* The BDO frame **dissolves** the hit-rate objection rather than answering it.
- **It is immune to the commodity objection.** *"Are we all chasing the same intel?"* has no meaning against labor — nobody asks whether two BDOs use the same phone book. A BDO's value is the judgment and work applied, not the raw universe. This is the same claim as the anti-commodity architecture (§3): the differentiation is the reasoning/labor layer, not the feed.
- **It keeps Quintel out of the CRM-replacement trap.** A BDO is not a CRM; it is a worker who *uses* one. This is the honest answer to "aren't you another private-credit CRM?" — no, we are the headcount you would hire to operate the CRM you already have. (*"I don't need a new HubSpot. I need this."* — Katharine.)

**The scope of the claim, stated honestly.** The agent does the *entry-level* BDO's work — list-tending, monitoring, enrichment, box-triage, net-new discovery, CRM hygiene, and the who-not-to-call inversion (the negative-signal / risk read, §8.1 of the signal portfolio). It does **not** do the senior originator's relationship, calling, judgment, and closing work. Sell the *defensible* claim — *"a tireless junior BDO that watches your whole list 24/7 and keeps it clean"* — and let the buyer draw the "replaces a hire" conclusion. Reserve "replace the entry-level BDO function" for the strategic/investor frame; do **not** point "we replace your people" at the person you are selling to (who often manages, or fears becoming, that headcount).

### The CRM integration is the proof of the BDO claim

A BDO's daily footprint is *in the CRM*: they read the prospect universe and tier-1s out of it and write net-new companies, status changes, and logged signals back into it. So:

- A feed *beside* the CRM is a tool — the originator still does the CRM work.
- An agent that *reads the customer's CRM, monitors it, enriches it, writes net-new similar companies back into it, and flags who has gone cold* is doing BDO work in the tool the BDO would use.

The CRM integration is therefore **not a roadmap feature — it is the demonstration of the entire BDO positioning.** The canonical loop is Katharine's own words: *"scrub our tier-ones and if something relevant comes up, tell us; if there's something similar but not in HubSpot, add it — a two-way street."* This pulls the live-CRM capability forward from v2 (§15) to **demo status**, delivered deliberately shallow (read a list + write back annotations and net-new; **not** production two-way sync). Build scope: [[quintel-empire-hubspot-bdo-demo-scope-2026-07-14]].

### The two-moat model (correcting the moat statement)

The prior moat statement (§8, §12) was *defensive* and conflated two different own-data moats:

- **Type 1 — per-customer lock-in.** The customer's own data makes *their* product better, so they don't leave. Real, but it is *switching cost*, not a competitive moat. (This is what the PRD has today; it is also what a dealer's telematics mostly is.)
- **Type 2 — pooled network effect.** Many customers' data, *anonymized and aggregated*, makes *everyone's* product better — so the product beats any competitor without the pool, and each customer is incentivized to contribute in order to get the pool back. This is the strongest moat that has ever existed in equipment finance: **PayNet** (hundreds of EF lenders contribute loss/performance data, receive risk scores + benchmarks back; now part of Equifax) proves that **EF lenders will contribute private data for pooled intelligence.** Filmore states the same model on its FAQ.

**The concrete Type-2 feature (Alek's insight).** Today the system infers a UCC signal's renewal window by *guessing* the loan term. If lenders contribute closed-deal data (equipment type + *actual* term financed), Quintel correlates signal patterns to observed term outcomes and tightens renewal-timing prediction **system-wide** — better for every customer as more contribute, unmatchable by any competitor without the pool.

**Reconciling with leak-safety (no architecture change).** There are two classes of contributed data, and the owner-tag model (§8) already fits both:
- **Raw `owner=P` deal history** (specific funded deals) → never crosses. Correct as-is; it is the ranking lock-in, and no one would contribute it raw.
- **Anonymized statistical aggregates** (signal→term correlations, benchmark distributions) → the Type-2 pool. The PRD already names this as the "shared calibration loop" (§8, §12) but files it as *deferred until scale.* The correction: **do not defer it conceptually.** It is the network moat, and it upgrades the moat story from defensive (lock-in) to offensive (a contributory EF-sourcing data cooperative).

**Two disciplines that keep this from becoming a distraction:**
1. **Scale-gated — not the v1 pitch or a v1 build.** At one customer there is no pool; pitching it would sell something that does not exist.
2. **Cheap to architect for now, expensive to retrofit.** Capture closed-deal metadata through the CRM pipe from day one, and put the anonymized-contribution model into the data design and the customer contract. **Architect now, build after N customers, pitch only when the pool is real.**

**The synthesis: the CRM integration is also the data-acquisition pipe for the pool.** The closed-won records that feed the Type-2 pool arrive through the *same* integration that delivers the BDO value — the two threads are one pipe. And the trust order falls out naturally: a lender lets an agent *read and enrich* its CRM (low trust) well before it *contributes outcomes to a pool* (high trust), so the BDO earns the trust that later unlocks the contribution.
...
**Sourcing-first re-weighting (2026-07-14).** Because the product now *leads* on sourcing, the **leading structured signals — permits, state DOT lettings, FMCSA new authority, gov contract awards** — are the actual *sourcing edge*: they fire *before* the deal, they're EF-specific and defensible, and much of the data is public/cheap. The original table buries several of these in L3 ("high effort, roadmap") while putting news at L2. That ordering optimizes for *density* (news makes the feed feel alive) over *edge* (leading signals make it early) — **density ≠ edge.** The [signal portfolio](quintel-data-strategy-signal-portfolio-2026-07-13.md) (§7 mis-prioritization, §10 sequencing) argues for **promoting the structured leading-signal feeds ahead of, or alongside, L2 news.** Treat that as the current direction: sequence leading signals *and* news; don't let news alone stand in for sourcing. *(This is a build-sequencing question for the engine roadmap — the four-level taxonomy below is unchanged; what changes is which levels are pulled forward and why.)*
...
- **Private personalization (v1) — the Type-1 moat:** every worked lead's outcome (won/lost + reason) re-fits the customer's own revealed-preference profile and boundary profile. Per-customer lock-in / switching cost. v1 fuel = the light manual outcome capture (§6.5).
- **Shared calibration — the Type-2 network moat (architect now, build at scale):** aggregate *anonymized* outcomes → system-wide intelligence (e.g., signal→actual-term correlations that tighten renewal-timing for *everyone*; PayNet-style benchmarks — Alek's insight, §4A). Scale- and privacy-gated, **not a v1 build** — but the contribution mechanism and contract language are designed in from day one so the CRM pipe captures closed-deal metadata. This is the *offensive* moat, distinct from the Type-1 lock-in.
- **Moat (two-part, §4A):** Type-1 per-customer lock-in (learned function + standing pipeline) **+** the public-research resolution/inference IP **+** the Type-2 pooled network effect the wedge earns into. **Raw private data never crosses; anonymized aggregates are the pool.**
...
> **Reframe caveat (2026-07-14).** Building the customer-data dial first is a fine *engineering* path (it's the cleanest thing to build and validate). But since the *product now leads on sourcing*, the entry-product case — **the Stream valuable on coarse context alone, no deal data** — is no longer an afterthought; it must be a first-class, proven state, not just a graceful-degradation fallback. Engineering should ensure step (1)'s ranker + Stream produce a *good* result with an empty/thin Book (base-prior + box + watchlist), and the demo should be able to show the coarse-context case, not only the rich-seeded-Book case. See §11 on whether leading structured signals should be pulled forward alongside L2.
...
> **Demo pull-forward (2026-07-14).** A deliberately *shallow* version of this integration — read a HubSpot company list + write back signal annotations, net-new similar companies, and who-not-to-call flags; **not** production two-way sync — is being built **now** as the Empire demo artifact, because the CRM integration is the *proof of the BDO framing* (§4A), not merely a v2 convenience. Build scope: [[quintel-empire-hubspot-bdo-demo-scope-2026-07-14]]. The full two-way sync described below remains v2.
...
**Settled:** a **sourcing-first relevance-stream** — the product leads on surfacing and ranking better leads from **market data run through the intelligence layer** (valuable with zero customer deal history), and the customer's own book is the *deepening* dial, not the entry point; the escape from the commodity trap is the **reasoning layer** (resolution + box-scoring + per-item "why"), not the raw feed and not, by itself, the customer's data. **Market intelligence is in v1** (L1 live; leading structured signals + L2 news pulled forward per §11 and the signal portfolio; L3/L4 depth on roadmap); the product is a ranked, annotated, interactive **Stream**, not a static list, validated on *comprehension* not accuracy; graceful degradation on deal-data depth (coarse-context floor → deal-data deepening); interaction controls as the immediate personalization loop; minimal private-entity graduation; no live CRM sync in v1 (synthetic CSV); light manual outcome capture; credit-memo extractor as the ingestion spine with base-fields + `extensions` + a thin per-customer adapter; CRM/notes data shape defined now for v2.
...
3. **Newsletter-collapse (now the #1 load-bearing risk of the reframe).** Leading on market data makes this the make-or-break assumption: the market content is commodity, so the *entire* differentiation is the *depth and visibility* of the per-customer relevance annotation + the interaction. If the tailoring is shallow ("filtered by industry"), Quintel *is* a high-priced newsletter — and now there's no customer-data-first framing standing in front of that objection; the reasoning layer has to carry it directly. *Mitigation (a hard execution bar, not a nice-to-have): even in the zero-deal-data case, the annotation must reference the originator's actual box/sponsors/sectors and connect them correctly, and the feed must visibly learn from interaction. This is the bar the demo must clear on a coarse-context account, not just a rich-book one.*
...
- `product/quintel/quintel-data-strategy-signal-portfolio-2026-07-13.md` — **the market-data/sourcing strategy this reframe elevates:** the five originator jobs (J1–J5), the leading-vs-coincident-vs-lagging temporal axis, UCC valued precisely, the gap/mis-prioritization analysis, and the sequencing recommendation (leading structured signals ahead of/alongside L2 news). Primary input to the 2026-07-14 sourcing-first reframe.
