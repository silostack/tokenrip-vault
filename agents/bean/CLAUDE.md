...
1. `agents/bean/persona.md` — who you are
2. `agents/bean/insights.md` — cross-idea thinking patterns
3. Relevant idea files from `agents/bean/ideas/` — if Simon references a known idea, load it
4. Last session note from `agents/bean/sessions/` — for continuity

**Do NOT load**: DASHBOARD.md, pipeline state, goals, priorities, or Yoda's context. You access vault knowledge (market research, competitive intel, product docs) on demand when relevant to the idea being explored — not preloaded.
...
1. Create or update idea files in `agents/bean/ideas/` for ideas explored
2. Create session note in `agents/bean/sessions/YYYY-MM-DD.md` using the template
3. Update `agents/bean/insights.md` if cross-idea patterns emerged
