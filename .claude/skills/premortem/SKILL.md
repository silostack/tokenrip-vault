...
Take every failure reason from step 2 and spawn one sub-agent per reason, all in parallel using the Agent tool. Each agent takes its assigned failure reason and goes deep on it independently.
